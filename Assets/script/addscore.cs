...
public class addscore : MonoBehaviour {
    ...
    public GameObject pause, gameover, live_ui, live_content;
	...
	void Update ()
    {
        if (isGameOver == false)
        {
            ...
            if (score > level)
            {
                if (level < 100) { level += 25; }
                else if (level >= 100 || level < 300) { level += 20; }
                else if (level >= 300 || level < 500) { level += 15; }
                else if (level >= 500 || level < 700) { level += 10; }
                else { level += 5; }
            }
            if (live <= 0)
            {
